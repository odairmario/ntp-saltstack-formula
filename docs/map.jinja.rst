...
We will see a quick setup to configure the ``ntp`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``ntp`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``ntp`` formulas under ``/srv/salt/ntp/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/ntp/parameters/dns:domain/
    ln -s dns:domain /srv/salt/ntp/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/ntp/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/ntp/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``ntp`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``ntp`` formulas under ``/srv/salt/ntp/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/ntp/parameters/roles
...
We create a configuration for the role ``ntp/server`` in ``/srv/salt/ntp/parameters/roles/ntp/server.yaml``:
...
We create another configuration for the role ``ntp/client`` in ``/srv/salt/ntp/parameters/roles/ntp/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles ntp/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles ntp/client
...
.. code-block:: console
    ...
    mv /srv/salt/ntp/parameters/dns:domain/  '/srv/salt/ntp/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-ntp-as-json:
      test.nop:
        - name: {{ ntp | json }}
...
Here is an example based on `template-formula/ntp/config/file.sls`_:
...
This ``sls`` file expose a ``ntp`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ ntp['winner'] }}
...
.. _template-formula/ntp/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/ntp/config/file.sls
